...
<body>
	...
	<form id="frm-member">
		...
		<div class="find-btn">
		<button type="button"  class="checkid-submit-btn find-btn1" >회원가입</button>
...
function loadedHandler() {
	...
	$(".checkid-submit-btn").click(btnCheckidSubmitClickHandler);
}

function btnCheckidSubmitClickHandler(){
    // 필수 입력사항 확인
    var id = $("#id").val();
    var pwd = $("#pwd").val();
    var pwdch = $("#pwdch").val();
    ...
    if (id === '' || pwd === '' || pwdch === '' || name === '' || phone === '' || email === '' || address === '') {
    ...
    $.ajax({
        ...
        success: function(result) {
            if (result > 0) {
                alert("사용불가!! 다른 아이디를 사용해주세요.");
                return;
            } else {
            	var frm = document.getElementById("frm-member");
            	frm.method="post";  // content 길이 길거라..
            	frm.action = "${pageContext.request.contextPath}/member";
            	frm.submit();
                alert("회원가입이 완료되었습니다."); 
            }
        },
